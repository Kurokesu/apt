...
The archive is a stateless, single-writer republisher: every run rebuilds the full index from `conf/manifest.yml` and verified release assets.

```mermaid
flowchart TD
  manifest["conf/manifest.yml"] --> ingest["scripts/ingest.sh"]
...
Releases are added manually. Add one block per distinct source tag to `releases:` in `conf/manifest.yml`. Filenames are derived, so you only supply `source`, `tag`, and `version` (and optionally `suites`).
